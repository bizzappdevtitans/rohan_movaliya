...
class SchoolStudent(models.Model):
    ...
    start_date = fields.Date(default=date.today(), tracking=True)
    last_date = fields.Date(compute="_compute_last_date")
    ...
    count_teacher = fields.Integer(compute="_compute_count_teacher")
    count_subject = fields.Integer(compute="_compute_count_subject")
    ...
    def _compute_no_of_boys(self):
        """compute method for count total number of girls #T00433"""
        for record in self:
            record.total_boys = 80 - record.total_girls

    def _inverse_total_no_of_boys(self):
        """inverse method to compute total number of boys #T00433"""
        for record in self:
